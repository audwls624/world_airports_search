...
try:
    with conn.cursor() as cursor:
        ...
        sql5 = '''CREATE TABLE nicknames (ID int NOT NULL auto_increment, name varchar(100), airport_id int, PRIMARY KEY (ID), FOREIGN KEY(airport_id) REFERENCES airports(ID))'''
        cursor.execute(sql1)
        cursor.execute(sql2)
        cursor.execute(sql3)
        cursor.execute(sql4)
